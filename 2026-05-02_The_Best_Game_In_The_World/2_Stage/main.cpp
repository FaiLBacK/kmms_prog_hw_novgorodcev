...
void init_object(Object* obj,
                 ...
                 float obj_height,
                 char cur_type);
bool is_collision(const Object* obj_1,
                  const Object* obj_2);
bool is_position_on_map(int x, int y);
void mario_collision(Object* mario,
                     Object** moving_objects,
                     int* moving_objects_count,
                     int* player_score,
                     int* current_level,
...
void set_object_pos(Object* obj,
                    float x_pos,
                    float y_pos);
...
int main()
{
    ...
    do
    {
        ...
        {
            Object obj = moving_objects[i];

            vertical_move_object(&obj,
                                 &brick,
                                 &brick_counts,
                                 &moving_objects,
                                 &moving_objects_count,
            ...
            moving_objects[i] = obj;
            put_object_on_map(map,
                              &moving_objects[i],
                              camera_x);
        }
    ...
    *camera_x = 0;
    ...
    if (lvl == 1)
    {
        ...
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    25, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    80, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
    }
    ...
    if (lvl == 2)
    {
        ...
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    65, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    120, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    160, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    175, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
    }
    ...
    if (lvl == 3)
    {
        ...
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    50, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    80, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
        ...
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    90, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    120, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
        ...
        init_object(get_new_moving_objects(moving_objects,
                                           moving_objects_count),
                    130, 10, ENEMY_WIDTH, ENEMY_HEIGHT,
                    TYPE_ENEMY);
    }
...
{
    (*moving_objects_count)--;
    (*moving_objects)[i] = (*moving_objects)
                           [*moving_objects_count];
    ...
    {
        Object* temp = static_cast<Object*>(
            realloc(*moving_objects,
...
void free_game_resources(Object** brick,
                         Object** moving_objects)
{
    if (brick != nullptr && *brick != nullptr)
    {
        free(*brick);
        *brick = nullptr;
    }

    if (moving_objects != nullptr &&
        *moving_objects != nullptr)
    {
        free(*moving_objects);
        *moving_objects = nullptr;
...
Object* get_new_brick(Object** brick, int* brick_counts)
{
    ...
    Object* temp = static_cast<Object*>(
        realloc(*brick, sizeof(Object) * (*brick_counts)));
                     ...
                     int max_level)
{
    for (int i = 0; i < *moving_objects_count; i++)
    {
        ...
        {
            if ((*moving_objects)[i].object_type ==
                TYPE_ENEMY)
            {
                float half_h = (*moving_objects)[i].height
                             * 0.5f;

                if ((mario->is_flying == true) &&
                    (mario->vertical_speed > 0) &&
                    (mario->y + mario->height <
                     (*moving_objects)[i].y + half_h))
                {
                       ...
                       float camera_x)
{
    const int ix = static_cast<int>(
        std::round(obj->x - camera_x));
    const int iy = static_cast<int>(std::round(obj->y));
    const int iWidth = static_cast<int>(
        std::round(obj->width));
    const int iHeight = static_cast<int>(
        std::round(obj->height));
...
{
    printf("=== МАРИО НА СИ++===\n");
    printf("Управление: A/D - движение, Пробел - "
           "прыжок, ESC - выход\n");
                          ...
                          float* camera_x)
{
    ...
    for (int i = 0; i < *brick_counts; i++)
    {
        if (is_collision(obj, &(*brick)[i]))
        {
            ...
            {
                (*brick)[i].object_type = '-';
                init_object(get_new_moving_objects(
                                moving_objects,
                                moving_objects_count),
                            (*brick)[i].x,
                            (*brick)[i].y - 3,
                ...
                (*moving_objects)[*moving_objects_count - 1]
                    .vertical_speed = COIN_DROP_SPEED;
            }
